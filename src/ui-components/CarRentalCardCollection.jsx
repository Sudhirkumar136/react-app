...
import { listCarDataModels } from "../graphql/queries";
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listCarDataModels.replaceAll("__typename", ""),
          variables,
        })
      ).data.listCarDataModels;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "CarRentalCardCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
